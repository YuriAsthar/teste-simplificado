...
{
    protected $signature = 'kafka:produce-transfer {payer_id} {payee_id} {amount_cents} {--dry-run : Show envelope without publishing}';
    ...
    private function validateArguments(int $payerId, int $payeeId, int $amountCents): void
    {
        if ($payerId <= 0 || $payeeId <= 0 || $amountCents <= 0) {
            throw new InvalidArgumentException('payer_id, payee_id, and amount_cents must be positive integers.');
        }
    }
}
