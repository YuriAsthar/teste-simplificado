...
|-------------|---------|------|
| `Api/V1/TransferControllerTest.php` | End-to-end tests for `POST /api/v1/transfer`: idempotency replay, fingerprint mismatch, transient 503, failed-transfer replay, missing-payer replay, empty idempotency key validation, and business-rule failures. | PHP |
| `Api/V1/TokenControllerTest.php` | Tests `POST /api/v1/auth/token` issuance. | PHP |
...
| `Database/Migrations/IdempotencyKeysMigrationBackfillTest.php` | Verifies the idempotency keys migration backfills `status` and `fingerprint` for legacy rows. | PHP |
