...
final readonly class KafkaTransferService
{
     ...
     */
    public function authorizeAndExecuteTransfer(int $payerId, int $payeeId, int $amount, Transfer $transfer): array
    {
        ...
        $this->dispatcher->dispatch(
            new SendNotificationJob($transferId)
        )->onConnection('rabbitmq');

        $this->invalidateUserCache($payerId);
        $this->invalidateUserCache($payeeId);

        $this->logger->info('Transfer completed event processed', [
            'transfer_id' => $transferId,
            'payer_id' => $payerId,
            'payee_id' => $payeeId,
            'amount' => $amount,
