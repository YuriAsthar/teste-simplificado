...
- **Retry semantics**: A `Failed` outbox event is eligible for retry if `attempts < max_attempts` AND `last_error_at` is older than `retry_interval_seconds`. The `markFailed()` method increments attempts, updates `last_error_at`, and explicitly sets `status = 'Failed'`. Retryability is kept in a single place (the `pending()` scope).
- **Concurrency**: `outbox:publish` is scheduled with `WithoutOverlapping` so it does not run concurrently in production. For extra safety a PostgreSQL advisory lock or row-level lock can be added around the batch loop.
